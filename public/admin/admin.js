...
/* v34 player picker remove final fix */
(function(){
  const $ = id => document.getElementById(id);
  const getToken = () => {
    try {
      return localStorage.getItem('pretzel_admin_token') || localStorage.getItem('pretzel_admin_session') || '';
    } catch(e){ return ''; }
  };
  ...
  function renderSelectablePlayers(players){
    ...
    if(!players || !players.length){
      box.innerHTML = '<div class="panel-note">No players loaded yet. Tap Load State.</div>';
      return;
    }
    ...
    for(const p of players){
      ...
      row.innerHTML = `<span style="display:inline-flex;align-items:center;justify-content:center;width:18px;height:18px;border-radius:6px;border:1px solid rgba(255,255,255,.18);margin-right:10px;font-size:12px;" data-check="1"></span><span>${p.name}</span>`;

      row.addEventListener('click', ()=>{
        const next = row.dataset.selected === '1' ? '0' : '1';
        row.dataset.selected = next;
        row.style.borderColor = next === '1' ? 'rgba(255,77,109,.45)' : 'rgba(255,255,255,.10)';
        row.style.background = next === '1' ? 'rgba(255,77,109,.10)' : 'rgba(0,0,0,.18)';
        const chk = row.querySelector('[data-check="1"]');
        if(chk) chk.textContent = next === '1' ? '✓' : '';
      });
  ...
  async function refreshSelectablePlayers(){
    ...
    try{
      if(out) out.textContent = 'Loading players...';
      const data = await adminFetch('getState', {});
      const st = data.state || data || {};
      const players = Array.isArray(st.players)
        ? st.players.slice().sort((a,b)=>String(a.name||'').localeCompare(String(b.name||'')))
        : [];
      renderSelectablePlayers(players);
      if(out) out.textContent = `${players.length} player(s) loaded`;
    }catch(e){
      if(out) out.textContent = e.message || 'Failed to load players';
    }
  }

  async function removeSelectedPlayers(){
    const out = $('bulkSelectResult');
    const ids = [...document.querySelectorAll('#bulkPlayerList [data-player-id][data-selected="1"]')].map(x => x.dataset.playerId);
    if(!ids.length){
    ...
    for(const id of ids){
      try{
        await adminFetch('deletePlayer', { playerId:id });
        removed++;
      }catch(e){
        failed++;
      }
    ...
    try{
      if (typeof toast === 'function') toast(`Removed ${removed} player(s)`);
    }catch(e){}
  }

  function selectAll(){
    document.querySelectorAll('#bulkPlayerList [data-player-id]').forEach(x=>{
      x.dataset.selected = '1';
      x.style.borderColor = 'rgba(255,77,109,.45)';
      x.style.background = 'rgba(255,77,109,.10)';
      const chk = x.querySelector('[data-check="1"]');
  ...
  function clearAll(){
    document.querySelectorAll('#bulkPlayerList [data-player-id]').forEach(x=>{
      ...
      x.style.borderColor = 'rgba(255,255,255,.10)';
      x.style.background = 'rgba(0,0,0,.18)';
      const chk = x.querySelector('[data-check="1"]');
      if(chk) chk.textContent = '';
    });
  }
  ...
  function bootPicker(){
    ...
    $('load')?.addEventListener('click', ()=>setTimeout(refreshSelectablePlayers, 250));
    $('selectAllPlayersBtn')?.addEventListener('click', selectAll);
    $('clearPlayersBtn')?.addEventListener('click', clearAll);
    $('removeSelectedPlayersBtn')?.addEventListener('click', removeSelectedPlayers);
  }

  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', bootPicker);
  else bootPicker();
})();
