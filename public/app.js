...
/*
 * Custom enhancement: move the live feed into its own tab and create tabs
 * on the front page.  This runs before the countdown/log fix so that
 * tab elements exist when other scripts attach event listeners.  By
 * wrapping in an immediately-invoked function expression (IIFE), we
 * avoid polluting the global scope.  Errors are caught to ensure this
 * enhancement never breaks the core functionality.
 */
(function(){
  try{
    // When the DOM is ready, restructure the page to add a board/feed tab bar
    document.addEventListener('DOMContentLoaded', function(){
      // Ensure the main wrapper has an ID so we can toggle it later
      var mainWrap = document.querySelector('main.wrap');
      if(mainWrap && !mainWrap.id){
        mainWrap.id = 'boardPanel';
      }
      // Insert tab buttons for Board and Live Feed if missing
      if(!document.getElementById('mainTabBoard')){
        var note = document.querySelector('.smallnote');
        var tabs = document.createElement('div');
        // reuse the .filters class so buttons match existing styling
        tabs.className = 'filters';
        var btnBoard = document.createElement('button');
        btnBoard.id = 'mainTabBoard';
        btnBoard.className = 'fbtn active';
        btnBoard.textContent = 'Board';
        var btnFeed = document.createElement('button');
        btnFeed.id = 'mainTabFeed';
        btnFeed.className = 'fbtn';
        btnFeed.textContent = 'Live Feed';
        tabs.appendChild(btnBoard);
        tabs.appendChild(btnFeed);
        // Insert after the small note (social link) for better layout
        if(note && note.parentNode){
          note.parentNode.insertBefore(tabs, note.nextSibling);
        }
      }
      // Move existing feed panel into its own container so it can be toggled
      if(!document.getElementById('feedPanel')){
        var feedEl = document.getElementById('feed');
        if(feedEl){
          // the feed sits inside a .panel wrapper; detach it from hero-right
          var panel = feedEl.parentNode;
          if(panel && panel.parentNode){
            panel.parentNode.removeChild(panel);
          }
          // create a new container for the feed panel and hide by default
          var feedContainer = document.createElement('div');
          feedContainer.id = 'feedPanel';
          feedContainer.style.display = 'none';
          feedContainer.appendChild(panel);
          // Insert the feed panel right after the main wrapper (board)
          if(mainWrap && mainWrap.parentNode){
            mainWrap.parentNode.insertBefore(feedContainer, mainWrap.nextSibling);
          } else {
    ...
    // Log errors silently to avoid breaking the site
    console.error('Tab enhancement error', e);
  }
})();
