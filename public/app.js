...
function playTutorial(platform){
  const steps = platform === "ios"
    ? [
        { title:"Open in Safari", body:"Open the game site in Safari (not Chrome).", tap:{x:165,y:365} },
        { title:"Add to Home Screen", body:"Tap Share → Add to Home Screen.", tap:{x:170,y:72} },
        { title:"Open from the icon", body:"Close Safari. Open the Pretzel HQ icon.", tap:{x:110,y:240} },
...
function enforceCountdownLock(){
  const start = new Date(GAME_START_ISO).getTime();
  const now = Date.now();
  if(now >= start || devUnlockValid()) return;

  const node = showOverlay(`
    <div class="overlay-card">
      <div class="overlay-head">
        <div>
          <div class="overlay-title">NBHS Senior Assassin</div>
          <div class="overlay-sub">Pretzel HQ unlocks when the game begins.</div>
        </div>
        <button class="linklike" id="cdUnlock">Admin/Test</button>
      </div>
      <div class="overlay-body">
        <div class="bigcount" id="cdBoxes"></div>
        <div class="smallnote" style="text-align:center;">March 9 • 3:30 PM (Central)</div>
        <div class="smallnote" style="margin-top:12px; text-align:center;">
          Install the app + enable notifications now, and you’ll be ready at launch.
        </div>
        <div class="pillrow" style="margin-top:14px;">
          <button class="pillbtn" id="cdHelp">Show install tutorial</button>
        </div>
      </div>
    </div>
  `);

  node.querySelector("#cdUnlock")?.addEventListener("click", async ()=>{ closeOverlay(node); await tryDevUnlock(); });
  node.querySelector("#cdHelp")?.addEventListener("click", ()=>{ closeOverlay(node); localStorage.removeItem("pretzel_onboard_done"); runOnboardingOnce(); });
  ...
  function render(){
    ...
    const boxes = [
      ...
      <div class="countbox">
        <div class="countnum">${String(val).padStart(2,"0")}</div>
        <div class="countlbl">${lbl}</div>
      </div>`).join("");

    const el = node.querySelector("#cdBoxes");
    if(el) el.innerHTML = boxes;
  ...
  const obs = new MutationObserver(()=>{ if(!document.body.contains(node)){ clearInterval(t); obs.disconnect(); }});
  obs.observe(document.body, { childList:true, subtree:true });
}

window.addEventListener('load', ()=>{ try{ runOnboardingOnce(); }catch{} try{ enforceCountdownLock(); }catch{} });
