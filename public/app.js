// --- GAME CONFIG ---
const GAME_START_ISO = "2026-03-09T15:30:00-06:00"; // March 9 @ 3:30 PM Central (edit if needed)
const DEV_UNLOCK_TTL_MS = 12 * 60 * 60 * 1000; // 12 hours
...
function demoState(){
  ...
  const teams = [
    { id:"t1", name:"Orange Squad" },
    { id:"t2", name:"Blue Squad" },
    { id:"t3", name:"Pretzel Ops" },
  ];
  const mk = (id,name,teamId,status,kills)=>({ id, name, teamId, status, kills, updatedAt: now });
  const players = [
    mk("p1","Ava","t1","alive",2),
    mk("p2","Ben","t1","alive",1),
    mk("p3","Cody","t1","pending",0),
    mk("p4","Dani","t1","eliminated",0),
    mk("p5","Eli","t1","alive",0),

    mk("p6","Finn","t2","alive",3),
    mk("p7","Gabe","t2","revived",1),
    mk("p8","Hana","t2","alive",0),
    mk("p9","Ivy","t2","eliminated",0),
    mk("p10","Jace","t2","alive",1),

    mk("p11","Kira","t3","alive",0),
    mk("p12","Liam","t3","pending",0),
    mk("p13","Maya","t3","alive",2),
    mk("p14","Noah","t3","alive",0),
    mk("p15","Owen","t3","eliminated",0),
  ];
  const log = [
    { id:"l1", ts: now, msg:"Preview data loaded (not the real game)." },
    { id:"l2", ts: now, msg:"Example: Finn gained a kill." },
    { id:"l3", ts: now, msg:"Example: Cody marked pending for review." },
  ];
  return { lastUpdated: now, teams, players, log, _demo:true };
}
...
function renderTimer(){
  ...
  const dev = devUnlockValid() && now < RELEASE_DATE;
  const unlocked = (now >= RELEASE_DATE) || dev;
  $("revealTitle").textContent = unlocked ? "HQ Live" : "Countdown Lock";
  $("revealBadge").textContent = unlocked ? "UNLOCKED" : "LOCKED";
  $("revealFoot").textContent = unlocked ? (dev ? "Preview mode (admin unlock)." : "Live board is active.") : "HQ unlocks March 9 at 3:30 PM (America/Chicago)";
...
async function fetchState(){
  ...
  const dev = devUnlockValid() && (Date.now() < RELEASE_DATE.getTime());
  if(dev && (!data?.teams?.length) && (!data?.players?.length)) return demoState();
  return data;
}
...
function overlayRoot(){ return document.getElementById("overlayRoot") || document.body; }
...
function closeOverlay(node){ try{ node?.remove(); }catch{} }
...
function runOnboardingOnce(){
  ...
  if(localStorage.getItem("pretzel_onboard_done")==="1") return;

  const picker = showOverlay(`
    <div class="overlay-card">
      <div class="overlay-head">
        <div>
          <div class="overlay-title">Install + Notifications</div>
          <div class="overlay-sub">Takes ~30 seconds. One-time setup.</div>
        </div>
        <button class="linklike" id="obSkip">Skip</button>
      </div>
      <div class="overlay-body">
        <div class="smallnote">Pick your device to see the right steps.</div>
        <div class="pillrow">
          <button class="pillbtn" id="obIOS">iPhone</button>
          <button class="pillbtn" id="obAndroid">Android</button>
        </div>
        <div class="smallnote" style="margin-top:10px;">
          Tip: Notifications only work after you add it to your Home Screen and open it from the icon.
        </div>
      </div>
    </div>
  `);
  ...
  const finish = ()=>{
    localStorage.setItem("pretzel_onboard_done","1");
    closeOverlay(picker);
  };

  picker.querySelector("#obSkip")?.addEventListener("click", finish);
  const play = (platform)=>{ closeOverlay(picker); playTutorial(platform); };
  picker.querySelector("#obIOS")?.addEventListener("click", ()=>play("ios"));
  picker.querySelector("#obAndroid")?.addEventListener("click", ()=>play("android"));
}

function playTutorial(platform){
  const steps = platform === "ios"
    ? [
        { title:"Tap Share", body:"In Safari, tap the Share icon (square with arrow).", tap:{x:165,y:452} },
        { title:"Add to Home Screen", body:"Scroll the list and tap Add to Home Screen.", tap:{x:160,y:362} },
        { title:"Tap Add", body:"Confirm by tapping Add in the top-right.", tap:{x:270,y:70} },
        { title:"Enable Notifications", body:"Open Pretzel HQ from your Home Screen and tap Enable Notifications.", tap:{x:165,y:316} }
      ]
    : [
        { title:"Open in Chrome", body:"Open the game site in Chrome.", tap:{x:165,y:365} },
        { title:"Install the app", body:"Tap ⋮ → Install app / Add to Home screen.", tap:{x:180,y:70} },
        { title:"Open from the icon", body:"Open Pretzel HQ from your Home Screen.", tap:{x:110,y:240} },
        { title:"Enable Notifications", body:"Inside the app, tap Enable Notifications.", tap:{x:120,y:300} }
      ];
  ...
  function renderDots(){
    return `<div class="dots">${steps.map((_,idx)=>`<div class="dot ${idx===i?'on':''}"></div>`).join("")}</div>`;
  }
  function render(){
    const s = steps[i];
    return `
      <div class="overlay-card">
        <div class="overlay-head">
          <div>
            <div class="overlay-title">${platform==="ios"?"iPhone Setup":"Android Setup"}</div>
            <div class="overlay-sub">${s.title}</div>
          </div>
          <button class="linklike" id="tutClose">Close</button>
        </div>
        <div class="overlay-body">
          <div class="stepwrap">
            <div class="phoneframe">
              <div class="phonebar"></div>
              <div class="tap" style="left:${s.tap.x}px; top:${s.tap.y}px;"></div>
            </div>
            ${renderDots()}
            <div class="smallnote" style="margin-top:10px;">${s.body}</div>
          </div>
          <div class="pillrow" style="margin-top:14px;">
            <button class="pillbtn" id="tutBack" ${i===0?'disabled style="opacity:.5;cursor:not-allowed"':''}>Back</button>
            <button class="pillbtn" id="tutNext">${i===steps.length-1?'Finish':'Next'}</button>
          </div>
          <div class="smallnote" style="margin-top:10px;">If you already installed it, you can close this.</div>
        </div>
      </div>
    `;
  }
  function rerender(){
    ...
    node.querySelector("#tutClose")?.addEventListener("click", ()=>{ localStorage.setItem("pretzel_onboard_done","1"); closeOverlay(node); });
    node.querySelector("#tutBack")?.addEventListener("click", ()=>{ if(i>0){ i--; rerender(); } });
    node.querySelector("#tutNext")?.addEventListener("click", ()=>{
      if(i < steps.length-1){ i++; rerender(); }
      else { localStorage.setItem("pretzel_onboard_done","1"); closeOverlay(node); }
    });
  }
  rerender();
}

function devUnlockValid(){
  const raw = localStorage.getItem("pretzel_dev_unlock") || "";
  if(!raw) return false;
  const ts = Number(raw.split("|")[0] || 0);
  if(!Number.isFinite(ts) || ts <= 0) return false;
  return (Date.now() - ts) < DEV_UNLOCK_TTL_MS;
}

async function tryDevUnlock(){
  const node = showOverlay(`
    <div class="overlay-card">
      <div class="overlay-head">
        <div>
          <div class="overlay-title">Testing Unlock</div>
          <div class="overlay-sub">Admin key required (only unlocks on your device)</div>
        </div>
        <button class="linklike" id="duClose">Close</button>
      </div>
      <div class="overlay-body">
        <div class="smallnote">Enter your admin key to view the app early for testing.</div>
        <div style="margin-top:10px; display:flex; gap:10px;">
          <input id="duKey" placeholder="Admin key" type="password"
            style="flex:1; padding:12px; border-radius:14px; border:1px solid rgba(255,255,255,.10); background:rgba(0,0,0,.22); color:var(--text); outline:none;">
          <button class="fbtn" id="duGo" style="padding:12px 14px;">Unlock</button>
        </div>
        <div class="smallnote" style="margin-top:10px;">Unlock expires in 12 hours.</div>
      </div>
    </div>
  `);
  node.querySelector("#duClose")?.addEventListener("click", ()=>closeOverlay(node));
  node.querySelector("#duGo")?.addEventListener("click", async ()=>{
    const key = node.querySelector("#duKey")?.value || "";
    try{
      const res = await fetch("/.netlify/functions/verifyAdmin", { headers: { "x-admin-key": key } });
      if(!res.ok) throw new Error("Invalid key");
      localStorage.setItem("pretzel_dev_unlock", `${Date.now()}|1`);
      closeOverlay(node);
      location.reload();
    }catch(e){ alert("Nope — key didn’t match."); }
  });
}

window.addEventListener('load', ()=>{
  try{ runOnboardingOnce(); }catch{}
  try{
    const params = new URLSearchParams(location.search);
    if(params.has('dev')){
      const a = document.getElementById('devUnlockLink');
      if(a){ a.style.display='inline-flex'; a.addEventListener('click', (e)=>{ e.preventDefault(); tryDevUnlock(); }); }
    }
  }catch{}
});
