...
function isAdmin(event){
  ...
  // ✅ Token auth (supports Authorization: Bearer <token> AND x-admin-token)
  const auth = (h["authorization"] || h["Authorization"] || "").toString().trim();
  const m = /^Bearer\s+(.+)$/i.exec(auth);
  const token = (m && m[1]) ? String(m[1]).trim() : String(h["x-admin-token"] || h["X-Admin-Token"] || "").trim();
  if (token){
    try{
...
async function loadState(){
  ...
  const base = raw || { lastUpdated:null, teams:[], players:[], log:[] };
  // Back-compat defaults
  if (!base.targets || typeof base.targets !== "object") base.targets = {};
  if (!base.metrics || typeof base.metrics !== "object"){
    base.metrics = { totalVisits:0, uniqueVisitIds:{}, playerLogins:0, uniquePlayerLogins:{} };
  }
  if (typeof base.metrics.totalVisits !== "number") base.metrics.totalVisits = 0;
  if (!base.metrics.uniqueVisitIds || typeof base.metrics.uniqueVisitIds !== "object") base.metrics.uniqueVisitIds = {};
  if (typeof base.metrics.playerLogins !== "number") base.metrics.playerLogins = 0;
  if (!base.metrics.uniquePlayerLogins || typeof base.metrics.uniquePlayerLogins !== "object") base.metrics.uniquePlayerLogins = {};
    if (!base.prize || typeof base.prize !== "object") base.prize = { amount:0, updatedAt:null, note:"", history:[] };
  if (typeof base.prize.amount !== "number") base.prize.amount = Number(base.prize.amount||0) || 0;
  if (!Array.isArray(base.prize.history)) base.prize.history = [];
  if (!base.killFeed || !Array.isArray(base.killFeed)) base.killFeed = [];
  return base;
}
